        ...
        <div class="flex space-x-8 overflow-x-auto pb-6">
            ...
            <div class="min-w-[340px] flex-shrink-0 p-8 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg card-hover smooth-transition border border-yellow-200">
                ...
                <div class="space-y-2 text-sm">
                    <p class="text-gray-600">Oldest pending: <span class="font-semibold text-gray-800">{{ $oldestPending ?? '5 days' }}</span></p>
                    <p class="text-gray-600">Average review time: <span class="font-semibold text-gray-800">{{ $avgReviewTime ?? '3 days' }}</span></p>
                </div>
            </div>
            ...
            <div class="min-w-[340px] flex-shrink-0 p-8 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg card-hover smooth-transition border border-green-200">
                ...
                <div class="space-y-2 text-sm">
                    <p class="text-gray-600">New approvals this month: <span class="font-semibold text-gray-800">{{ $newApproved ?? '10' }}</span></p>
                    <p class="text-gray-600">Average onboarding time: <span class="font-semibold text-gray-800">{{ $avgOnboardingTime ?? '2 days' }}</span></p>
                </div>
            </div>
            ...
            <div class="min-w-[340px] flex-shrink-0 p-8 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg card-hover smooth-transition border border-red-200">
                ...
                <div class="space-y-2 text-sm">
                    <p class="text-gray-600">Common reasons: <span class="font-semibold text-gray-800">{{ $commonRejectionReasons ?? 'Missing documents, expired licenses' }}</span></p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($applications ?? [] as $application)
                            <tr class="table-row smooth-transition cursor-pointer">
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $application->ref_number ?? 'APP-2024-001' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-800">{{ $application->name ?? 'John Smith' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $application->profession ?? 'Registered Nurse' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-600">{{ $application->submitted_at ?? '2024-05-28' }}</td>
                                <td class="px-6 py-4 text-center">
                                    <button onclick="openModal('{{ $application->ref_number }}', '{{ $application->name }}', '{{ $application->email }}', '{{ $application->phone ?? '+1 (555) 123-4567' }}', '{{ $application->address ?? '123 Main St, City, State 12345' }}', '{{ $application->profession }}', '{{ $application->specialization ?? 'ICU/Critical Care' }}', '{{ $application->experience ?? '5 years' }}', '{{ $application->start_date ?? '2024-06-15' }}', '{{ $application->resume_url ?? '#' }}', '{{ $application->license_url ?? '#' }}', '{{ $application->certifications_url ?? '#' }}', '{{ $application->snapshot_url ?? '/snapshot/APP-2024-001' }}', '{{ $application->pdf_url ?? '/snapshot/APP-2024-001.pdf' }}')" class="btn-secondary px-6 py-2 rounded-lg text-sm font-semibold">
                                        View Details
                                    </button>
                                </td>
                            </tr>
                ...
                <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-200">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-800">{{ $application->work_type ?? 'Full-time, Part-time' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-medium text-gray-600">Shift Type:</span>
                            <span class="text-gray-800">{{ $application->shift_type ?? 'Day shift, Night shift' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-medium text-gray-600">Location:</span>
                            <span class="text-gray-800">{{ $application->location ?? 'Metro Area' }}</span>
                        </div>
                    </div>
                </div>
