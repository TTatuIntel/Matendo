...
if ($request_id <= 0 || $assignee_id <= 0 || empty($description) || empty($due_date) || empty($priority) || empty($status)) {
    echo json_encode(['status' => 'error', 'message' => 'All fields are required']);
    exit;
}

$stmt = $conn->prepare("INSERT INTO care_tasks (request_id, assignee_id, description, due_date, priority, status) VALUES (?, ?, ?, ?, ?, ?)");
$stmt->bind_param("iissss", $request_id, $assignee_id, $description, $due_date, $priority, $status);
