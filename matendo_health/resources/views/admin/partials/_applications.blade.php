                        ...
                        @forelse ($applications as $application)
                            <tr class="table-row smooth-transition cursor-pointer">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <button onclick="openModal('{{ $application->reference_number }}', '{{ $application->first_name }} {{ $application->last_name }}', '{{ $application->email }}', '{{ $application->phone }}', '{{ $application->address }}', '{{ $application->profession }}', '{{ $application->specialization ?? 'none' }}', '{{ $application->years_experience }} years', '{{ $application->start_date?->format('Y-m-d') ?? 'N/A' }}', '{{ $application->resume ? route('admin.applications.download', [$application->id, 'resume']) : '#' }}', '{{ $application->license_doc ? route('admin.applications.download', [$application->id, 'license']) : '#' }}', '{{ $application->certifications ? route('admin.applications.download', [$application->id, 'certifications']) : '#' }}', '{{ $application->id }}')" class="btn-secondary px-4 py-2 rounded-full text-sm font-semibold flex items-center mx-auto">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
