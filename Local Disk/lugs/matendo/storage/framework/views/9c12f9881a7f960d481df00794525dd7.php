...
<div class="p-8 space-y-8">
            ...
            <div class="min-w-[340px] flex-shrink-0 p-8 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg card-hover smooth-transition border border-yellow-200">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-yellow-400 to-amber-400 text-yellow-900 text-sm font-bold rounded-full shadow-sm">
                        <?php echo e($pendingApplications ?? 30); ?>

                    </span>
                </div>
                <p class="text-gray-700 mb-4 text-base">Applications waiting for admin review and verification.</p>
                <div class="space-y-2 text-sm">
                    <p class="text-gray-600">Oldest pending: <span class="font-semibold text-gray-800"><?php echo e($oldestPending ?? '5 days'); ?></span></p>
                    <p class="text-gray-600">Average review time: <span class="font-semibold text-gray-800"><?php echo e($avgReviewTime ?? '3 days'); ?></span></p>
                </div>
            </div>
            ...
            <div class="min-w-[340px] flex-shrink-0 p-8 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg card-hover smooth-transition border border-green-200">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-400 to-emerald-400 text-green-900 text-sm font-bold rounded-full shadow-sm">
                        <?php echo e($approvedApplications ?? 85); ?>

                    </span>
                </div>
                <p class="text-gray-700 mb-4 text-base">Applications successfully verified and approved for platform access.</p>
                <div class="space-y-2 text-sm">
                    <p class="text-gray-600">New approvals this month: <span class="font-semibold text-gray-800"><?php echo e($newApproved ?? '10'); ?></span></p>
                    <p class="text-gray-600">Average onboarding time: <span class="font-semibold text-gray-800"><?php echo e($avgOnboardingTime ?? '2 days'); ?></span></p>
                </div>
            </div>
            ...
            <div class="min-w-[340px] flex-shrink-0 p-8 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg card-hover smooth-transition border border-red-200">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-red-400 to-rose-400 text-red-900 text-sm font-bold rounded-full shadow-sm">
                        <?php echo e($rejectedApplications ?? 8); ?>

                    </span>
                </div>
                <p class="text-gray-700 mb-4 text-base">Applications declined due to incomplete or invalid information.</p>
                <div class="space-y-2 text-sm">
                    <p class="text-gray-600">Common reasons: <span class="font-semibold text-gray-800"><?php echo e($commonRejectionReasons ?? 'Missing documents, expired licenses'); ?></span></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div>
        ...
        <div class="flex justify-end mb-6 space-x-4">
            <button class="btn-primary px-6 py-3 rounded-xl text-base font-semibold flex items-center">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                </svg>
                Refresh
            </button>
            <button class="btn-secondary px-6 py-3 rounded-xl text-base font-semibold flex items-center">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php $__currentLoopData = $applications ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $application): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="table-row smooth-transition cursor-pointer">
                                <td class="px-6 py-4 text-sm font-medium text-gray-900"><?php echo e($application->reference_number ?? 'APP-2024-001'); ?></td>
                                <td class="px-6 py-4 text-sm text-gray-800"><?php echo e($application->full_name ?? 'John Smith'); ?></td>
                                <td class="px-6 py-4 text-sm text-gray-700"><?php echo e($application->profession ?? 'Registered Nurse'); ?></td>
                                <td class="px-6 py-4 text-sm text-gray-600"><?php echo e($application->created_at->format('Y-m-d') ?? '2024-05-28'); ?></td>
                                <td class="px-6 py-4 text-center">
                                    <button onclick="openModal('<?php echo e($application->reference_number); ?>', '<?php echo e($application->full_name); ?>', '<?php echo e($application->email); ?>', '<?php echo e($application->phone ?? '+1 (555) 123-4567'); ?>', '<?php echo e($application->address ?? '123 Main St, City, State 12345'); ?>', '<?php echo e($application->profession); ?>', '<?php echo e($application->specialization ?? 'ICU/Critical Care'); ?>', '<?php echo e($application->years_experience ?? '5 years'); ?>', '<?php echo e($application->start_date ? $application->start_date->format('Y-m-d') : '2024-06-15'); ?>', '<?php echo e($application->resume ? Storage::url($application->resume) : '#'); ?>', '<?php echo e($application->license_doc ? Storage::url($application->license_doc) : '#'); ?>', '<?php echo e($application->certifications ? Storage::url($application->certifications) : '#'); ?>', '<?php echo e(route('admin.applications.snapshot-pdf', $application->id)); ?>', '<?php echo e(route('admin.applications.redownload-pdf', $application->id)); ?>')" class="btn-secondary px-6 py-2 rounded-lg text-sm font-semibold">
                                        View Details
                                    </button>
                                </td>
                            </tr>
                ...
                <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-200">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-800"><?php echo e($application->work_type ?? 'Full-time, Part-time'); ?></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-medium text-gray-600">Shift Type:</span>
                            <span class="text-gray-800"><?php echo e($application->shift_type ?? 'Day shift, Night shift'); ?></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-medium text-gray-600">Location:</span>
                            <span class="text-gray-800"><?php echo e($application->preferred_location ?? 'Metro Area'); ?></span>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // Modal handling
    function openModal(refNumber, name, email, phone, address, profession, specialization, experience, startDate, resumeUrl, licenseUrl, certificationsUrl, snapshotUrl, pdfUrl) {
        currentApplicationData = { refNumber, name, email, snapshotUrl, pdfUrl };
        ...
        document.getElementById('modalLicense').href = licenseUrl;
        document.getElementById('modalCertifications').href = certificationsUrl;
        ...
        approveButton.onclick = () => openApprovalPopup(refNumber, name, email, snapshotUrl, pdfUrl);
        rejectButton.onclick = () => handleReject(refNumber);
    }
    ...
    function openApprovalPopup(refNumber, name, email, snapshotUrl, pdfUrl) {
        document.getElementById('popupEmail').textContent = email;
        document.getElementById('popupEmailInput').value = email;
        document.getElementById('popupPassword').textContent = '********';
        document.getElementById('popupPasswordInput').value = '********';
    ...
    // Reject application
    async function handleReject(refNumber) {
        const url = `/admin/applications/${refNumber}/reject`;
...
<?php $__env->stopPush(); ?><?php /**PATH D:\lugs\matendo\resources\views/admin/partials/_applications.blade.php ENDPATH**/ ?>